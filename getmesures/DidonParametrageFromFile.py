...
class DidonParametrageFromFile: # Définition de la classe DidonParametrage
...
## ######################################
    ...
    def __init__(self, envToParam): # Notre méthode constructeur
        ...
        if envToParam == ENV_PROD:
            ## parametrage de PROD
            self.XRuser = conf["@@XRUSERPROD@@"]
            self.XRpwd = conf["@@XRPWDPROD@@"]
            self.XRhost = conf["@@XRHOSTPROD@@"]
            self.XRbase = conf["@@XRDBPROD@@"]

            self.DIDONuser = conf["@@DIDONUSERPROD@@"]
            self.DIDONpwd = conf["@@DIDONPWDPROD@@"]
            self.DIDONhost = conf["@@DIDONHOSTPROD@@"]

            self.DIDONlogDir = conf["@@DIDONLOGDIRPROD@@"]
        else :
            ## parametrage LOCAL et DEFAUT
            self.XRuser = conf["@@XRUSERLOCAL@@"]
            self.XRpwd = conf["@@XRPWDLOCAL@@"]
            self.XRhost = conf["@@XRHOSTLOCAL@@"]
            self.XRbase = conf["@@XRDBLOCAL@@"]

            self.DIDONuser = conf["@@DIDONUSERLOCAL@@"]
            self.DIDONpwd = conf["@@DIDONPWDLOCAL@@"]
            self.DIDONhost = conf["@@DIDONHOSTLOCAL@@"]

            self.DIDONlogDir = conf["@@DIDONLOGDIRLOCAL@@"]

        self.DIDONbase = conf["@@DIDONDB@@"]
        self.DIDONschemaMesure = conf["@@DIDONSCHEMA@@"]
        ...
        self.getDidonLogger().ecrireLog( nomClasse, nomFonction,'Initialisation du parametrage terminee')
...
## ######################################
    ...
    def afficheParametrages(self):
        ...
        parametrage =  'Parametres de connexion XR : \n'
        parametrage += 'XRuser='+str(self.getXRuser())+'\n'
        parametrage += 'XRpwd='+str(self.getXRpwd())+'\n'
        parametrage += 'XRhost='+str(self.getXRhost())+'\n'
        parametrage += 'XRbase='+str(self.getXRbase())+'\n \n'

        parametrage += 'Parametres de connexion DIDON : \n' 
        parametrage += 'DIDONuser='+str(self.getDIDONuser())+'\n'
        parametrage += 'DIDONpwd='+str(self.getDIDONpwd())+'\n'
        parametrage += 'DIDONhost='+str(self.getDIDONhost())+'\n'
        parametrage += 'DIDONbase='+str(self.getDIDONbase())+'\n \n'

        parametrage += 'Parametres généraux : \n' 
        parametrage += 'Noms courts des mesures pris en compte : '+str(self.getNomsCourtsMesures())+'\n'
        parametrage += 'Fréquences de mesure autorisées : '+str(self.getFrequencesMesure())+'\n'
        parametrage += 'Nb annees à traiter (cas freq=A): '+str(self.getNbAnneesATraiter())+'\n'
        parametrage += 'Dernière annee à traiter (cas freq=A): '+str(self.getDerniereAnneeATraiter())+'\n'
        parametrage += 'Nb jours à traiter (cas freq=H,J,M) : '+str(self.getNbJoursATraiter())+'\n'
        parametrage += 'Arrondis configurés : '+str(self.getDictArrondis())+'\n'
        parametrage += 'Taux de representativité configurés : '+str(self.getDictRepresentativites())+'\n'
        parametrage += 'Fréquences de rééchantillonnage : '+str(self.getDictFreqResampling())+'\n'
        ## affichage console
        print(parametrage)
        ##log
        self.getDidonLogger().ecrireLog( nomClasse, nomFonction,parametrage)

    def readAndGetConf(self):
        dictConfig = {}
        with open("DidonParametrage.conf") as file :
            for line in file :
                ##print(line)
                cle,val=line.split("=")
                ##print("--->clé="+cle.strip()+" ; valeur="+val.strip())
                dictConfig[cle.strip()]=val.strip()
        file.close()
        ##print("!!! dictConfig="+str(dictConfig))
        return dictConfig
