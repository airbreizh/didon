...
import psycopg2
from DidonParametrageFromFile import DidonParametrageFromFile
...
## ######################################
    ...
    def __init__(self, envToParam, freqMesure): # Notre méthode constructeur
        ...
        self.parametrage = DidonParametrageFromFile(envToParam)
...
## ######################################
    ...
    def isFrequenceAutorisee(self,freq):
        """
            ...
            Si 
                la fréquence passée en parametre est présente dans le paramétrage DidonParametrageFromFile.frequencesMesure 
            Alors retourne True
            Sinon retourn False
        """
    ...
    def connectXR(self):
        """
            ...
            Permet d'établir la connexion XR via la classe pyair.xair.XAIR avec le paramétrage de DidonParametrageFromFile
    ...
    def connectDidon(self):
        """
            ...
            Permet d'établir la connexion à la base de données DIDON avec le paramétrage de DidonParametrageFromFile
            pour un environnement PROD ou LOCAL
        """
    ...
    def afficheStatut(self, etape):
        ...
        if etape == LABEL_INITIALISATION:
            ...
            statut += '\n'
            statut += 'Fréquence à traiter : '+str(self.frequenceMesure)+'\n'
            statut += 'Table à utiliser : '+str(self.getTableAUtiliser())+'\n'

        statut += 'Etat des connexions : \n'
        statut += 'Connexion XR : '+str(self.getXRConnected()) +'\n'
        statut += 'Connexion DIDON : '+str(self.getDIDONConnected())+'\n'
        ...
        self.getParametrage().getDidonLogger().ecrireLog( nomClasse, nomFonction,'')
        self.getParametrage().getDidonLogger().ecrireLog( nomClasse, nomFonction,'')
        self.getParametrage().getDidonLogger().ecrireLog( nomClasse, nomFonction,statut)
        self.getParametrage().getDidonLogger().ecrireLog( nomClasse, nomFonction,'')
        self.getParametrage().getDidonLogger().ecrireLog( nomClasse, nomFonction,'')
